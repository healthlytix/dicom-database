...
class Keycloak:
    ...
    def get_user_profile_from_token(self, jwt_token: str) -> UserProfileResponse:
        ...
        if 'RD-NAME' in role_config.authorized_labels and decoded_token['family_name'] and decoded_token['given_name']:
            label_override_value='RD-'+decoded_token['family_name']+'-'+decoded_token['given_name']
            rendered_authorize_labels = [ label_override_value if label == 'RD-NAME' else label for label in role_config.authorized_labels ]
            logging.info(f"label_override_value={label_override_value} rendered_authorize_labels={rendered_authorize_labels}")
        else:
            rendered_authorize_labels = role_config.authorized_labels
            logging.info(f"rendered_authorize_labels={rendered_authorize_labels}")

        response.authorized_labels = rendered_authorize_labels
